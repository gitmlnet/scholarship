...
| 5 Mock backend | Full route table, persistence, ID/duplicate logic, payment simulation, state machine, audit log; wizard wired to services | Service/API test suite green incl. isolation | ✅ done 2026-09-07 — full §7 surface: auth (salted-SHA256 credentials, opaque sessionStorage sessions w/ enforced 8h expiry, constant-time compare, login/logout/me + audit), public tracking `GET /applications/:id/status` (anti-enumeration: status/payment/timestamps only, identical 404s), `/me/applications` (owner-scoped), admin list (status/grade/q filters, 3 sorts, 10/page) + detail + `PATCH …/status` (state machine `APPLICATION_TRANSITIONS` in types, illegal→422 with allowed set, admin payment adjudication) + `POST …/correction` + audit log (record filter, 20/page, 500-entry rotation); DemoPay simulation (2s delay, tx-ids ending `00` flagged; settle-on-read + timer, idempotent, reload-safe); applications table seeds 2 settled demo apps over `db.v1` storage; audit actions for create/status/payment/correction/auth; MockApiClient awaits async handlers + attaches session token (FetchApiClient sends `Authorization: Bearer`); MockServer shapes async-handler rejections; wizard already service-wired; lint/typecheck/212 tests/build green |
