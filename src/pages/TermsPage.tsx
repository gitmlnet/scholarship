...
/** Terms & conditions of the fictional program. */
export default function TermsPage() {
  const { t } = useTranslation(['pages', 'common']);

  useDocumentMeta(t('terms.title'), t('terms.description'));
  ...
  return (
    <PageShell title={t('terms.title')} description={t('terms.description')}>
      <div className="max-w-3xl">
        <Alert variant="info" title={t('terms.intro')} />

        <div className="mt-10 space-y-10">
          {sections.map((section, index) => (
            <section key={section.heading} aria-labelledby={`terms-section-${index}`}>
              <h2
